...
(function () {
  ...
  function mockDetail(d, record) {
    ...
    const ipd = !pa && approved >= 5000;
    const treatment = ipd ? 'IPD' : 'OPD';
    ...
    return {
      ...
      claimType: treatment, claimStatus: 'Close',
      school: pa ? first(d.school, 'โรงเรียนวัฒนาศึกษา') : undefined,
      hospital: 'โรงพยาบาลสินแพทย์', branch: 'กรุงเทพมหานคร',
  ...
  function render(d, goBack) {
    ...
    const theme = createTheme({
      ...
      palette: { primary: { main: '#006bb3' }, success: { main: '#087443' }, warning: { main: '#a85200' }, error: { main: '#c62828' }, text: { primary: '#273b50', secondary: '#61758b' } },
      shape: { borderRadius: 8 },
      components: { MuiButton: { defaultProps: { disableElevation: true }, styleOverrides: { root: { textTransform: 'none' } } }, MuiTableCell: { styleOverrides: { root: { fontFamily: 'var(--ui-font, Sarabun, sans-serif)', fontSize: 16, padding: '12px 16px', borderBottom: '1px solid #e2eaf1' }, head: { fontWeight: 600, background: '#edf5fc', color: '#245477', whiteSpace: 'nowrap' } } } }
    });
    ...
    const section = (id, title, icon, body) => h(Box, { component: 'section', id: 'ccro-' + id, className: 'ccro-section', 'aria-labelledby': 'ccro-heading-' + id },
      h(Stack, { direction: 'row', alignItems: 'center', gap: 1.2, sx: { mb: 2 } }, h('span', { className: 'material-icons-round', 'aria-hidden': true, style: { color: '#006bb3' } }, icon), h(Typography, { component: 'h2', id: 'ccro-heading-' + id, fontSize: 18, fontWeight: 700, color: 'primary.main' }, title)), body);
    ...
    root.render(h(ThemeProvider, { theme }, h(Box, { className: 'ccro-shell' },
      h(Box, { className: 'ccro-backline' }, h(Button, { onClick: goBack, startIcon: h('span', { className: 'material-icons-round', 'aria-hidden': true }, 'arrow_back'), sx: { mb: 1 } }, 'กลับ'), d.mock && h(Typography, { variant: 'caption', color: 'text.secondary' }, 'ข้อมูลตัวอย่าง • อ่านอย่างเดียว')),
      h(Box, { component: 'section', className: 'ccro-hero', 'aria-label': 'สรุปเคลมลูกค้า' },
        h(Box, { className: 'ccro-identity' },
          ...
          pa && h(Typography, { sx: { mt: .5, fontSize: 16 } }, 'สถานศึกษา : ' + text(d.school))
        ),
        ...
        h(Box, { className: 'ccro-refs' }, fields([['Claim No', d.claimNo], ['Case No', d.caseNo], ['Application ID', d.appId], ['สถานะเคลม', d.claimStatus]], { '& dt': { color: '#d0e5fa' }, '& dd': { color: '#fff' } }))
      ),
      h(Paper, { variant: 'outlined', className: 'ccro-quick', sx: { mt: 1.5, p: 2, boxShadow: 'none' } }, fields([['วันที่แจ้งเคลม', d.notifiedDate], ['วันที่พิจารณา', first(decision.date, d.decisionDate)], ['ผู้พิจารณา', first(decision.user, d.reviewer)], ['สถานพยาบาล', d.hospital], ['สาขา', d.branch]])),
      h(Box, { component: 'nav', 'aria-label': 'ส่วนรายละเอียดเคลม', sx: { display: 'flex', overflowX: 'auto', py: 1, my: .5, gap: .5 } }, navigation.map(([id, label]) => h(Button, { key: id, component: 'a', href: '#ccro-' + id, sx: { flexShrink: 0 }, onClick: e => { e.preventDefault(); document.getElementById('ccro-' + id).scrollIntoView({ block: 'start' }); } }, label))),
      h(Paper, { variant: 'outlined', sx: { boxShadow: 'none', overflow: 'hidden' } },
  ...
  async function open(record, insured, history) {
    ...
    if (!root) node.innerHTML = '<p role="status" style="padding:24px">กำลังโหลดรายละเอียดเคลม…</p>';
    window.scrollTo(0, 0);
    try {
      await ready();
