/* ============================================================
   ...
   - พิจารณาเคลมลูกค้า: เริ่มต้นเป็น "ไม่ใช่" และแสดงรายละเอียดที่เลือกได้เมื่อเป็นอุบัติเหตุจราจร
...
(function () {
  ...
  function stateFor(page) {
    ...
    if (!stateByClaim.has(key)) {
      ...
      stateByClaim.set(key, {
        isTrafficAccident: editable ? "no" : "yes",
  ...
  function buildSection(page) {
    ...
    body.dataset.trafficDetails = "true";
    body.hidden = editable && state.isTrafficAccident !== "yes";

    if (editable) {
      const accidentGroup = create("fieldset", "hospital-traffic-filter");
      accidentGroup.dataset.trafficGroup = "accident";
      const accidentLegend = create("legend");
      appendRequired(accidentLegend, "เป็นอุบัติเหตุจากการจราจร");
      const accidentErrorId = prefix + "TrafficAccidentError";
      const accidentOptions = create("div", "hospital-traffic-filter-options");
      accidentOptions.append(
        radioOption(prefix + "TrafficAccident", "yes", "ใช่", state.isTrafficAccident === "yes", accidentErrorId, true),
        radioOption(prefix + "TrafficAccident", "no", "ไม่ใช่", state.isTrafficAccident === "no", accidentErrorId, true)
      );
      accidentGroup.append(accidentLegend, accidentOptions, errorSlot(accidentErrorId));
      section.appendChild(accidentGroup);
    }
  ...
  function clearTrafficDetailState(section, state, prefix) {
    state.vehicle = "";
    state.vehicleOther = "";
    state.casualty = "";
    state.poroboExcess = "";
    state.noPoroboReason = "";
    section.querySelectorAll('.hospital-traffic-body input[type="radio"]').forEach(function (input) {
      input.checked = false;
      input.setAttribute("aria-invalid", "false");
    });
    section.querySelectorAll(".hospital-traffic-body .hospital-traffic-group").forEach(function (group) {
      group.classList.remove("is-invalid");
      setError(group.querySelector(":scope > .hospital-traffic-error"), "");
    });
    [prefix + "TrafficVehicleOther", prefix + "TrafficNoPoroboReason"].forEach(function (id) {
      const field = section.querySelector("#" + id);
      if (field) field.value = "";
      clearInvalid(field);
      if (field?.parentElement) field.parentElement.hidden = true;
    });
  ...
  function bindSection(page, section, state) {
    section.addEventListener("change", function (event) {
      ...
      if (name.endsWith("TrafficAccident")) {
        state.isTrafficAccident = field.value;
        const details = section.querySelector("[data-traffic-details='true']");
        if (state.isTrafficAccident !== "yes") clearTrafficDetailState(section, state, pagePrefix(page));
        if (details) details.hidden = state.isTrafficAccident !== "yes";
        const summary = section.querySelector(".hospital-traffic-summary");
        if (summary && state.isTrafficAccident !== "yes") {
          summary.hidden = true;
          summary.textContent = "";
        }
      } else if (name.endsWith("TrafficVehicle")) {
  ...
  function validateTraffic(page, shouldFocus) {
    ...
    if (state.isTrafficAccident !== "yes") {
      const summary = section.querySelector(".hospital-traffic-summary");
      if (summary) {
        summary.hidden = true;
        summary.textContent = "";
