...
(function () {
  ...
  function claimCategoryMarkup() {
    return `
      <section id="claimCategorySection" class="claim-option-section claim-category-section rounded-2xl border p-4">
        ...
        <div class="claim-category-options" role="radiogroup" aria-label="ประเภทเคลม">
          <label class="claim-category-card" data-claim-category-card="${CUSTOMER}">
            <input class="sr-only claim-category-input" type="radio" name="claimCategory" value="${CUSTOMER}">
            <span class="claim-category-icon"><span class="material-icons-round" aria-hidden="true">person</span></span>
            <span><span class="claim-category-title">เคลมลูกค้า</span></span>
            <span class="claim-category-check material-icons-round" aria-hidden="true">check_circle</span>
          </label>
          <label class="claim-category-card" data-claim-category-card="${HOSPITAL}">
            <input class="sr-only claim-category-input" type="radio" name="claimCategory" value="${HOSPITAL}">
            <span class="claim-category-icon"><span class="material-icons-round" aria-hidden="true">local_hospital</span></span>
            <span><span class="claim-category-title">เคลมโรงพยาบาล</span></span>
            <span class="claim-category-check material-icons-round" aria-hidden="true">check_circle</span>
          </label>
        </div>
      </section>`;
  ...
  function syncCategoryCards() {
    ...
    document.querySelectorAll("#claimCategorySection .claim-category-input").forEach(function (input) {
      ...
      input.closest(".claim-category-card")?.classList.toggle("is-selected", input.checked);
    });
  }
  ...
  function renderHospitalFacility() {
    ...
    host.innerHTML = `
      <label class="hospital-facility-field md:col-span-3" for="claimHospitalSelect">
        ...
        <span id="claimHospitalHelp" class="hospital-facility-help"><span class="material-icons-round !text-lg" aria-hidden="true">info</span><span>ข้อมูลนี้จะถูกส่งไปยังคิวพิจารณาเคลมโรงพยาบาล</span></span>
