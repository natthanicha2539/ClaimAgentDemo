(function () {
  ...
  function renderQuickFilters() {
    ...
    return statusOptions.map(function (option) {
      ...
      const tone = option.value === "ALL" ? "all" : getStatusMeta(option.value).tone;
      return '<button type="button" class="hat-quick-filter is-' + tone + (active ? " is-active" : "") + '" data-hat-quick-filter="' + option.value + '" aria-pressed="' + String(active) + '"><span>' + option.label + '</span><b>' + counts[option.value] + "</b></button>";
    }).join("");
  }
