(function () {
  ...
  function getDecisionDraft(request) {
    if (!state.decisionDrafts[request.requestId]) {
      const approvedAmount = (request.treatmentItems || []).reduce(function (total, item) {
        return total + Number(item.approveNow || 0);
      }, 0);
      state.decisionDrafts[request.requestId] = {
        decision: approvedAmount < request.requestedAmount ? "partial" : "approve",
        approvedAmount: formatAdditionalTransferMoney(approvedAmount),
        notCoveredAmount: formatAdditionalTransferMoney(Math.max(0, request.requestedAmount - approvedAmount)),
  ...
  function renderDetailSectionHeader(icon, title, description, actionMarkup) {
    return '<div class="hat-section-header"><div class="hat-section-title"><span class="hat-section-icon material-icons-round" aria-hidden="true">' + icon + '</span><div><h3>' + title + '</h3><p>' + description + "</p></div></div>" + (actionMarkup || "") + "</div>";
  }
  ...
  function renderTreatmentItems(request) {
    return (request.treatmentItems || []).map(function (item) {
      ...
      return '<article class="hat-treatment-item"><div class="hat-treatment-heading"><div><strong>' + escapeHtml(item.name) + '</strong><span>' + escapeHtml(item.code) + '</span></div><span class="hat-check-result ' + (success ? "is-success" : "is-warning") + '"><span class="material-icons-round" aria-hidden="true">' + (success ? "check_circle" : "warning") + "</span>" + escapeHtml(item.result) + '</span></div><div class="hat-treatment-values">' +
        '<div><span>ยอดตามใบเสร็จเดิม</span><strong>' + formatAdditionalTransferMoney(item.receiptOld) + "</strong></div>" +
        '<div><span>สิทธิ์อนุมัติเดิม</span><strong>' + formatAdditionalTransferMoney(item.approvedOld) + "</strong></div>" +
        '<div><span>ยอดที่ขอเพิ่ม</span><strong>' + formatAdditionalTransferMoney(item.requested) + "</strong></div>" +
        '<div class="is-current"><span>ยอดพิจารณารอบนี้</span><strong>' + formatAdditionalTransferMoney(item.approveNow) + "</strong></div>" +
      "</div></article>";
    }).join("");
  }
  ...
  function renderDocuments(request) {
    ...
    return documents.map(function (documentItem) {
      return "<tr><td><strong>" + escapeHtml(documentItem.name) + "</strong></td><td>" + escapeHtml(documentItem.fileName) + '</td><td class="hat-align-center"><span class="hat-doc-count">' + Number(documentItem.count || 0) + '</span></td><td><span class="hat-doc-detail">' + escapeHtml(documentItem.detail) + '</span></td></tr>';
    }).join("");
  }
  ...
  function renderDecisionSummary(request) {
    ...
    const notCoveredAmount = parseAdditionalTransferMoney(draft.notCoveredAmount);
    const decisionLabels = {
      approve: "อนุมัติโอนเพิ่ม",
      partial: "อนุมัติบางส่วน",
      return: "ส่งกลับแก้ไข",
      reject: "ปฏิเสธการโอนเพิ่ม"
    };
    const requestGap = Math.max(0, request.requestedAmount - approvedAmount);
    ...
    if (!summary) return;
    summary.innerHTML = '<div class="hat-calc-row is-result"><span>ผลการพิจารณา</span><strong>' + decisionLabels[draft.decision] + '</strong></div>' +
      '<div class="hat-calc-row"><span>โรงพยาบาลขอเพิ่ม</span><strong>' + formatAdditionalTransferMoney(request.requestedAmount) + " บาท</strong></div>" +
      '<div class="hat-calc-row"><span>ยอดอนุมัติโอนเพิ่ม</span><strong>' + formatAdditionalTransferMoney(approvedAmount) + " บาท</strong></div>" +
      '<div class="hat-calc-row"><span>ยอดไม่คุ้มครอง</span><strong>' + formatAdditionalTransferMoney(notCoveredAmount) + " บาท</strong></div>" +
      '<div class="hat-calc-row"><span>ส่วนต่างที่ไม่ได้อนุมัติ</span><strong>' + formatAdditionalTransferMoney(requestGap) + " บาท</strong></div>" +
      '<div class="hat-calc-row is-total"><span>ยอดอนุมัติสะสมใหม่</span><strong>' + formatAdditionalTransferMoney(cumulativeApproved) + " บาท</strong></div>";
  }
  ...
  function bindDetailEvents(request) {
    ...
    const notCoveredAmount = document.getElementById("hatNotCoveredAmount");
    ...
    heroStickyButton?.addEventListener("click", function () {
      ...
      const nextSticky = state.heroStickyByRequest[request.requestId] === false;
      ...
      heroStickyButton.setAttribute("aria-label", nextSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero");
      heroStickyButton.setAttribute("title", nextSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero");
      ...
      if (label) label.textContent = nextSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero";
    });
    ...
    function captureDetailState() {
      ...
      if (notCoveredAmount) draft.notCoveredAmount = notCoveredAmount.value;
    ...
    if (!decision || !approvedAmount || !notCoveredAmount || !remark) return;
    decision.addEventListener("change", function () {
      draft.decision = decision.value;
      renderDecisionSummary(request);
    });
    approvedAmount.addEventListener("input", function () {
      draft.approvedAmount = approvedAmount.value;
      renderDecisionSummary(request);
    });
    notCoveredAmount.addEventListener("input", function () {
      draft.notCoveredAmount = notCoveredAmount.value;
      renderDecisionSummary(request);
    });
    remark.addEventListener("input", function () { draft.remark = remark.value; });
    [approvedAmount, notCoveredAmount].forEach(function (input) {
      input.addEventListener("blur", function () {
        input.value = formatAdditionalTransferMoney(parseAdditionalTransferMoney(input.value));
        if (input === approvedAmount) draft.approvedAmount = input.value;
        if (input === notCoveredAmount) draft.notCoveredAmount = input.value;
        renderDecisionSummary(request);
      });
    });
  }
  ...
  function renderAdditionalTransferDetail(request) {
    ...
    mount.innerHTML = '<section class="hat-detail-hero' + (heroSticky ? "" : " is-unpinned") + '" aria-labelledby="hatDetailHeroTitle"><div class="hat-hero-toolbar"><button type="button" class="hat-back-button" data-hat-back-monitor><span class="material-icons-round" aria-hidden="true">arrow_back</span>กลับไปหน้ามอนิเตอร์</button><button type="button" class="hat-hero-pin-button" data-hat-toggle-hero-sticky aria-pressed="' + String(heroSticky) + '" aria-label="' + (heroSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero") + '" title="' + (heroSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero") + '"><span class="material-icons-round" aria-hidden="true">' + (heroSticky ? "push_pin" : "keep_off") + '</span><span class="hat-hero-pin-label">' + (heroSticky ? "ยกเลิกการตรึง Hero" : "ตรึง Hero") + '</span></button></div><div class="hat-hero-grid"><div class="hat-hero-main"><div class="hat-hero-tags"><span>' + product + ' · เคลมโรงพยาบาล</span><span>' + escapeHtml(request.treatment) + '</span></div><h2 id="hatDetailHeroTitle">' + escapeHtml(request.insured) + '</h2><div class="hat-hero-hospital"><span>สถานพยาบาล</span><strong>' + escapeHtml(request.hospital) + '</strong></div><div class="hat-hero-reference-grid"><div><span>Claim No.</span><strong>' + escapeHtml(request.claimNo) + '</strong></div><div><span>Case No.</span><strong>' + escapeHtml(request.caseNo) + '</strong></div><div><span>แผนความคุ้มครอง</span><strong>' + escapeHtml(request.plan) + '</strong></div></div></div><div class="hat-hero-side"><div class="hat-hero-statuses"><div><span>สถานะเคส</span><strong class="hat-case-status"><span class="material-icons-round" aria-hidden="true">autorenew</span>' + escapeHtml(request.caseStatus || "Re-Open") + '</strong></div><div><span>สถานะคำขอ</span><strong class="hat-request-status is-' + requestStatus.tone + '"><span class="material-icons-round" aria-hidden="true">' + requestStatus.icon + "</span>" + requestStatus.label + '</strong></div></div><div class="hat-hero-amounts"><div><span>ยอดเงินอนุมัติครั้งก่อน</span><strong>' + formatAdditionalTransferMoney(request.originalApproved) + '</strong><em>บาท</em></div><div class="is-requested"><span>ยอดเงินที่ขอโอนเพิ่ม</span><strong>' + formatAdditionalTransferMoney(request.requestedAmount) + '</strong><em>บาท</em></div></div></div></div></section>' +
      ...
      '<section class="hat-detail-section">' + renderDetailSectionHeader("compare_arrows", "รายการค่ารักษาที่ขอพิจารณาเพิ่ม", "เปรียบเทียบยอดเดิม ยอดที่ขอเพิ่ม และยอดพิจารณารอบนี้") + '<div class="hat-treatment-list">' + renderTreatmentItems(request) + '</div></section>' +
      ...
      '<section class="hat-detail-section">' + renderDetailSectionHeader("description", "เอกสารเคลม", "เอกสารประกอบคำขอโอนเพิ่มแบบอ่านอย่างเดียว") + '<div class="hat-doc-table-shell overflow-x-auto soft-scroll"><table class="hat-doc-table"><thead><tr><th>รายการเอกสาร</th><th>ไฟล์เอกสาร</th><th class="hat-align-center">จำนวนเอกสาร</th><th>รายละเอียด</th></tr></thead><tbody>' + renderDocuments(request) + '</tbody></table></div></section>' +
      '<section class="hat-detail-section">' + renderDetailSectionHeader("gavel", "ผลการพิจารณา", "บันทึกแบบร่างการพิจารณาสำหรับคำขอนี้") + '<form class="hat-decision-form" novalidate><label><span>ผลการพิจารณา</span><select id="hatDecisionSelect"><option value="approve"' + (draft.decision === "approve" ? " selected" : "") + '>อนุมัติโอนเพิ่ม</option><option value="partial"' + (draft.decision === "partial" ? " selected" : "") + '>อนุมัติบางส่วน</option><option value="return"' + (draft.decision === "return" ? " selected" : "") + '>ส่งกลับแก้ไข</option><option value="reject"' + (draft.decision === "reject" ? " selected" : "") + '>ปฏิเสธการโอนเพิ่ม</option></select></label><label><span>ยอดอนุมัติโอนเพิ่ม</span><div class="hat-amount-input"><input id="hatApprovedAmount" inputmode="decimal" value="' + escapeHtml(draft.approvedAmount) + '"><span>บาท</span></div></label><label><span>ยอดไม่คุ้มครอง</span><div class="hat-amount-input"><input id="hatNotCoveredAmount" inputmode="decimal" value="' + escapeHtml(draft.notCoveredAmount) + '"><span>บาท</span></div></label><label><span>ผู้พิจารณา</span><input value="' + escapeHtml(request.latestApprover) + '" readonly></label><label class="is-wide"><span>เหตุผล / หมายเหตุ</span><textarea id="hatDecisionRemark" rows="4">' + escapeHtml(draft.remark) + '</textarea></label></form></section>' +
      '<section class="hat-detail-section hat-decision-summary-section" id="hatDecisionSummary">' + renderDetailSectionHeader("calculate", "สรุปผลการพิจารณา", "สรุปยอดจากแบบร่างของคำขอนี้") + '<div id="hatDecisionSummaryValues" class="hat-calc-list"></div></section>';
