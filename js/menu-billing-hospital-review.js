  ...
  function renderBillingDocumentCheck(){
    return `<div class="rounded-xl border border-slate-200 bg-white p-4 billing-doc-check-section">
      ...
      <div class="overflow-x-auto rounded-xl border border-slate-200">
        <table class="w-full min-w-[720px] border-collapse text-sm billing-doc-check-table">
          ...
          <tbody id="billingReviewDocRows">${renderDocRows(false)}</tbody>
        </table>
      </div>
    </div>`;
  }

  function renderDocRows(includeReviewColumns = true){
    return currentBillingReview.docs.map((d,i)=>{
      ...
      return `<tr class="border-b border-slate-200 bg-white hover:bg-brand-50/40">
        ...
        ${includeReviewColumns ? `<td class="px-3 py-3 align-middle"><div class="flex flex-wrap items-center justify-center gap-2">${button('ผ่าน','ผ่าน','pass')}${button('ไม่ผ่าน','ไม่ผ่าน','fail')}${button('รอเอกสารเพิ่มเติม','รอเอกสารเพิ่มเติม','wait')}</div></td>
...
/* ---- billing-hospital-approval-to-fund-v1: ย้ายรายการที่อนุมัติไปตั้งเบิกกองทุน ---- */
(function(){
  'use strict';
  if(window.__billingHospitalApprovalToFundV1) return;
  window.__billingHospitalApprovalToFundV1 = true;
  ...
  const rows = () => {
    try { return window.hospitalBillingRows || (typeof hospitalBillingRows !== 'undefined' ? hospitalBillingRows : []); }
    catch(e) { return window.hospitalBillingRows || []; }
  };
  const activeReviewRow = () => {
    const current = window.currentConsiderHospitalRow || {};
    const claimNo = clean(current.claimNo || current.claimCode || window.__hospitalBillingClaimNo || window.__hospitalBillingClaimNo);
    const id = clean(current.id || window.__hospitalBillingReviewRowId);
    return rows().find(row => (id && clean(row.id) === id) || (claimNo && clean(row.claimNo) === claimNo)) || null;
  };
  const isApprovalButton = button => {
    if(!button || button.tagName !== 'BUTTON') return false;
    if(!button.closest('#billingHospitalReviewPage')) return false;
    if(!button.closest('#hhStepPane3,#hfStepPane3')) return false;
    const label = clean(button.textContent).replace(/check_circle/g,'');
    return /อนุมัติ/.test(label) && !/อนุมัติแล้ว/.test(label);
  };
  function showMovedToast(row){
    document.getElementById('billingHospitalFundMovedToast')?.remove();
    const toast = document.createElement('div');
    toast.id = 'billingHospitalFundMovedToast';
    toast.setAttribute('role','status');
    toast.setAttribute('aria-live','polite');
    toast.style.cssText = 'position:fixed;top:22px;right:22px;z-index:100002;display:flex;gap:10px;align-items:flex-start;min-width:330px;max-width:460px;padding:14px 16px;border:1px solid #86efac;border-radius:14px;background:#f0fdf4;color:#166534;box-shadow:0 16px 36px rgba(15,23,42,.18);font-family:var(--ui-font,sans-serif)';
    toast.innerHTML = `<span class="material-icons-round" style="font-size:26px!important;color:#16a34a">check_circle</span><div><strong style="display:block;font-size:15px">อนุมัติรายการสำเร็จ</strong><span style="display:block;margin-top:3px;font-size:13px;color:#35634a">รายการถูกย้ายไปหน้า ตั้งเบิกกองทุน > เคลมโรงพยาบาลแล้ว</span><b style="display:block;margin-top:3px;font-size:12px">${clean(row?.caseNo || row?.claimNo || '-')}</b></div>`;
    document.body.appendChild(toast);
    setTimeout(() => toast.remove(), 3600);
  }
  function moveToFundQueue(){
    try {
  ...
  function approveAndMove(event, button){
    const row = activeReviewRow();
    if(!row || row.status === 'รอตั้งเบิก') return false;
    event.preventDefault();
    event.stopImmediatePropagation();
    row.status = 'รอตั้งเบิก';
    row.itemStatus = 'รอตั้งเบิก';
    row.approvedBillingDate = row.approvedBillingDate || todayThai();
    row.approvedBy = row.approvedBy || 'Login User';
    row.fundQueueAt = todayThai();
    if(window.currentConsiderHospitalRow){
      window.currentConsiderHospitalRow.status = 'รอตั้งเบิก';
      window.currentConsiderHospitalRow.itemStatus = 'รอตั้งเบิก';
    }
    button.dataset.hospitalApproveCompletedV1 = 'true';
    button.disabled = true;
    button.setAttribute('aria-disabled','true');
    button.innerHTML = '<span class="material-icons-round mr-1">check_circle</span>อนุมัติแล้ว';
    showMovedToast(row);
    setTimeout(moveToFundQueue, 700);
    return true;
  }
  function install(){
...
(function(){
  ...
  function removeBillingDocumentReviewColumns(pane){
    pane.querySelectorAll('table').forEach(table => {
      const headers = Array.from(table.querySelectorAll('thead th'));
      const labels = headers.map(header => cleanLabel(header.textContent));
      if(!labels.some(label => label.includes('รายการเอกสาร'))) return;
      const indexes = labels
        .map((label,index) => ({label,index}))
        .filter(item => item.label === 'ผลการตรวจ' || item.label === 'หมายเหตุ')
        .map(item => item.index)
        .sort((a,b) => b-a);
      ...
      table.dataset.billingDocumentColumnsTrimmed = 'true';
  ...
  function normalizeBillingStep1(){
    ...
    removeBillingDocumentReviewColumns(pane);
