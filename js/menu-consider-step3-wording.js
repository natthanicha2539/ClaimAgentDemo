(function () {
  ...
  function isCustomerCompensationPane(pane) {
    var page = owningPage(pane);
    return Boolean(
      page &&
      page.id === 'considerCustomerDetailPage' &&
      window.customerClaimSelectionState &&
      String(window.customerClaimSelectionState.coverage || '').trim() === 'ค่าชดเชย'
    );
  }

  function updateHospitalExpenseSummary(pane) {
    if (isCustomerCompensationPane(pane)) {
      pane.querySelectorAll('[data-claim-step3-receipt-total]').forEach(function (row) { row.remove(); });
      return 0;
    }
